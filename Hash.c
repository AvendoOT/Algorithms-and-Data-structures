...
int Upis (struct zapis ulaz, FILE *ft, int prim) {
  ...
  poc = i;
  do { // Ponavljaj dok ne upi�e� ili ustanovi� da je datoteka puna    
...
void Ispis (FILE *ft) {
  // Ispis sadr�aja tablice raspr�enih adresa
  int i, j;
...
int Trazi (char mbr[], FILE *ft, int prim, struct zapis *ulaz) {
  ...
  // Upamti izra�unatu adresu kao po�etnu
  poc = i;
  do {  // Ponavljaj dok ne nadje� ili ustanovi� da ga nema
    ...
    // Pretinac je pun, prije�i cikli�ki na sljede�ega
    i = (i + 1) % M;
  } while (i != poc); // Dok se ne ne vrati� na po�etni
	return 0; // Svi pretinci posje�eni, zapis nije prona�en
}
...
int main() {
  ...
  // �itaj slijedno JMBG, prezime, ime dok ima podataka
	getchar ();
